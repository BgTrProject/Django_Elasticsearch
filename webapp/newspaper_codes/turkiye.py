...
class turkiye:
    ...
    def creator(self,url):
        ...
        try:
            ...
            # content_array = soup.find("div", attrs={"class": "js - news - content news - detail__content font - setting font - setting"}).getText()
            content_array = soup.find_all('p')

            # content_array = content_array.split()
            content_string = ""
            for w in content_array:
                content_string = content_string + " " + w.get_text()
        except:
            content_string = "None"
        try:
            date = soup.find("div", attrs={"class": "news-detail__status"}).getText()
        ...
        w_data = "{} ; {} ; {} ; {}".format(url.strip(), date.strip(), title.strip(),
                                            content_string.strip())  # category çıkarıldı.
        print(w_data)
        # write_to_txt(w_data)
...
# import io
# from pprint import pprint
# url="https://www.turkiyegazetesi.com.tr/gundem/704605.aspx"
# r = requests.get(url)
# soup = BeautifulSoup(r.content, 'html5lib')
# try:
#     title = soup.find("h1").getText()
...
#
#     # w_data = url+";"+date+";"+title+";"+content_string
# w_data = "{} ; {} ; {} ; {}".format(url.strip(), date.strip(), title.strip(), content_string.strip()) #category çıkarıldı.
# print(w_data)
